...
import space.aiyo.var.CrudMessage;
...
public class MongoWrapper extends AbstractVerticle {
    ...
    private Handler<Message<CrudMessage>> delete() {
        return message -> client.removeDocuments(message.body().getDocumentName(), message.body().getQuery(), result -> {
            if (result.succeeded()) {
                message.reply(result.result());
            } else {
    ...
    private Handler<Message<CrudMessage>> count() {
        return message -> client.count(message.body().getDocumentName(), message.body().getQuery(), result -> {
            if (result.succeeded()) {
                message.reply(result.result());
            } else {
